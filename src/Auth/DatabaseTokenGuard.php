...
class DatabaseTokenGuard implements GuardContract
{
    ...
    {
        return (resolveApp()->isLocal() ? '' : '__Host-').Str::slug(mustConfigString('app.name').'_'.mustConfigString('app.env').'_'.static::$cookieName."_{$this->guardName}", '_');
    }
    ...
    public function queueCookie(DatabaseToken $databaseToken): void
    {
        ...
        $cookieJar->queue($cookieJar->forever($this->cookieName(), $databaseToken->bearer, '/', null, true, true, false, resolveApp()->isProduction() ? 'Lax' : 'None'));
    }
